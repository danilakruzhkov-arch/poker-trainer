...
- `poker-trainer.html` — всё приложение. `APP_VER` ~строка 1134 (текущая `2026-07-24.28`).
- `wrap.js` — билд.
...
**Статус:** P1(а) free/paid split — СДЕЛАНО (.25–.28). Дальше P1(б): роль «редактор» (RLS для не-админских почт на `pack`) + очередь модерации на `feedback`. Требует новых миграций (Данила применяет руками) и продуктовых решений о правах редактора — делать с ревью, не вслепую. Отдельный спринт до P2.1 — переезд на домен `clippoints.com` + свой сервер (сейчас GitHub Pages).
