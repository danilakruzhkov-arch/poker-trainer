...
- `poker-trainer.html` — всё приложение. `APP_VER` ~строка 1134 (текущая `2026-07-24.29`).
- `wrap.js` — билд.
...
**Статус:** P1(а) free/paid split — СДЕЛАНО (.25–.28). P1(б) роль «редактор» + экран модерации — КЛИЕНТ СДЕЛАН (.29), **ждёт применения `deploy/supabase-roles-moderation.sql`** Данилой (до этого спит, билд = админ-only). Редактор = только паки. Дальше P2 монетизация: старт с Telegram Mini App (ТГ-логин через `initData`, платежи Telegram Stars → вебхук бота → `grant_pack`). Переезд на свой сервер — НЕ блокер для ТГ (Pages https подходит под Mini App), домен позже (`clippoints.com` Данила забраковал для этого проекта).
